        ...
        while True:
            if AI:
                ...
                model, function = get_model_and_running_func(
                    weights_path=str(os.path.join(settings.WEIGHTS_DIR, 'best.pt')),
                    data_path=str(os.path.join(settings.DATA_DIR, 'data.yaml')))
                frame, img = camera.get_frame_with_detection(function, model, threshold,
